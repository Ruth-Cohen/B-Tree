...
public:
	...
	BNode();
	BNode(T x); //initialize as a fleaf
...
//private:
	...
	void DeleteLast(T &val, BNode<T>* &lastSubTree); //delete the last in the node. returns the value deleted and the last SubTree
	void DeleteValueFromLeaf(int ToDelete); //delete a value from a leaf
	void unifyWith(int brotherInd, int holeInd); //unify values and pointers with values and pointers of Son[brotherInd],
	//and then delete Son[brotherInd]
...
template <typename T>
void BNode<T>::DeleteLast(T &val, BNode<T>* &lastSubTree) //delete the last in the node. returns the value deleted and the first SubTree
...
public:
	...
	BNode<T>* search(T x) { return search(root, x); }
	BNode<T>* search(BNode<T> *p, T x);
...
//this recurtion function deletes the sub tree.
template <typename T>
void BTree<T>:: deleteSubTree(BNode<T> *t)
{
	if (t != NULL)//if the tree is not empty
	{
		for (int i = 0; i < (t->nsons) ; i++)//scanning till the end of the sons' list
			deleteSubTree(t->Son[i]);//deletes all the sons.
		for (int i = 0; i < ((t->nsons) -1); i++)//deletes the values of the current node.
			delete t->Son[i];
	}
	return;
}
...
template <typename T>
BNode<T>* BTree<T>:: search(BNode<T> *p, T x)
{
		BNode<T>*  tmp;
		if (p != NULL)
		{
			for (int i = 0; i < p->nkeys; i++)//search in the current node's values (keys).
			{
				if (p->Key[i] == x)//found.
					return p;
			}
			for (int i = 0; i < p->nsons; i++)//now go and search in the array of the soons.
			{
				tmp=search(p->Son[i], x);//keep the answer in tmp.
				if (tmp != NULL)//you found.
					return tmp;
			}	 
		}
		return NULL;//in a case it is a leaf (last leaf=not found) or empty tree.  
	}